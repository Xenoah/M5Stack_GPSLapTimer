...
void showvalue(int dulation) {
  if (millis() > lastdulation + dulation)
  {
    ...
    if (LapCount > 1)
    {
      ...
      M5.Lcd.print(">");
      M5.Lcd.setTextSize(6);
      M5.Lcd.print((millis() - BeforeTime) / 1000, 3);
    }
    /*
        //**********2コ前ラップ表示
        if (LapCount > 1)
    ...
    */
    ...
    if (AverageLap != 0)
    {
      M5.Lcd.fillRect(10, 200, 300 * ((AverageLap - (millis() - BeforeTime) / 1000) / AverageLap), 25, PINK);//平均タイム(ピンク)
    }
    if (BestLap != 99999)
    {
      M5.Lcd.fillRect(10, 200, 300 * ((BestLap - (millis() - BeforeTime) / 1000) / BestLap), 25, CYAN);//ベストラップ(シアン)
    }
